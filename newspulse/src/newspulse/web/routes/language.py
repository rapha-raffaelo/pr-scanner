...
def set_language(code: str, request: Request) -> Response:
    ...
    # absolute URL here would make this an open redirect. The check is shared
    # because three routes had each written their own and two were wrong.
    target = redirects.local_target(request.query_params.get("next"))
