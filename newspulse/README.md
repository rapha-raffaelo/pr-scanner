...
**The topic radar** — a second Google News search per mandate, built from its
*themes* (keywords, then alert topics) instead of its name. It answers a different
question from the two sources above: not "who wrote about my client?" but "what
happened in their market?". Those items are stored as ordinary articles but are
never filed as coverage of the client — the client is not in them — and they exist
to feed the positioning drafts below. A mandate with no keywords and no alert
topics gets no radar, and competitors never get one.

Set `NEWSPULSE_GOOGLE_NEWS=0` to run on the curated registry alone (this also
switches off the radar, since both are Google News searches).
...
## Impulse (positioning drafts)

The third column on **Heute**. Where Empfehlungen answers "what should we do about
our coverage?", this answers the question a consultant is actually paid for: *the
market just moved — is there something this client should be saying about it, and
what?*

It runs off the topic radar, so the trigger is coverage the client is **not** in.
That is the point: by the time a mandate is named in an article, the moment to
position has usually passed. Each draft carries

- the **message**, ready to send: prose, no salutation, one click to the clipboard,
- the **thesis** it rests on, and the **overclaim** it deliberately is not,
- the **context**, the client's **credibility** on the subject, two to four
  derivable **statements**, and links to the developments behind it.

The overclaim field is not decoration. A draft that cannot name the sweeping
version of its own claim is usually making it.

Generated by the daily sweep, one model call per mandate whose radar found
something new — nothing is asked for a mandate with no fresh material, and dedup
means yesterday's development is not pitched again this morning. Failures are
logged and never mark the sweep as degraded: a missed opening is not a broken run.

Same posture as everything else here: **advisory, never autonomous.** Nothing is
sent anywhere, and most days for most mandates the honest answer is an empty
column — the prompt says so explicitly.
...
- `advisories` — one row per generated brief (situation + suggestions as JSON).
- `angles` — one row per positioning draft (message, context, thesis, overclaim,
  statements, and the article ids it cites). Kept apart from `advisories` because
  the two point in opposite directions: an advisory is an internal to-do list, an
  angle is outward-facing text.
